...
def get_body(wp: wiki.Page, lib: rs.Library, cat: str) -> str:
    ...
    if wp.parents: wp.body = md.n(f'> Main pages: ' + ', '.join([md.replace(i, lib.names) for i in wp.parents])) + wp.body
    if wp.smashable: wp.body = wp.body + f'\nThis object **doesn\'t drop itself** when broken.  \n'
    ...
    wp.body = wp.body + get_stats(wp.raw, lib, (
        {'slotCount': {}},  # Object params
        {'threat': {'name': 'Threat level'}},  # Weather params
        {'blockingStat': {}, 'defaultDuration': {'metric': 's'}, 'effectConfig': {'name': 'Effect parameters'}},  # Effect params
        {'statusEffects': {}},
        {'health': {}, 'shape': {}, 'middleMinSize': {}, 'middleMaxSize': {}},  # Plant params
        {'fixedStem': {'name': 'Stem/Stipe'}, 'fixedFoliage': {'name': 'Foliage/Cap'}},  # Sapling params
        {'mainBlock': {'no_dmg': True}, 'subBlocks': {'name': 'Secondary block options', 'no_dmg': True, 'non_empty': True}},  # Biome params
    ))
    if wp.surface.get('all'): wp.body = wp.body + '\n### Surface\n' + get_placeable_desc(wp.surface, lib)
    if wp.caves.get('all'): wp.body = wp.body + '\n### Underground\n' + get_placeable_desc(wp.caves, lib)
    wp.body = wp.body + get_abils_desc(wp, lib, '### ')  # Abilities
    if pages := wp.get('contentPages', []):  # Codex
        if wp.wiki.get('preserveContents'):
            codex = '```\n' + md.clean('\n\n'.join(['\n'.join(page.split('  ')) for page in pages])) + '\n```\n'
        else:
            codex = md.enrich('\n\n'.join(['  \n'.join(page.split('  ')) for page in pages]), lib.names) + '\n'
...
def get_placeable_desc(items: dict, lib: rs.Library, body=''):
    if objs := items.get('objects'): body = body + '\n**Objects**: ' + get_placeable_item(objs, lib) + '  '
    if objs := items.get('treasure'): body = body + '\n**Treasure**: ' + get_placeable_item(objs, lib) + '  '
    if items.get('bushes') or items.get('grasses') or items.get('treetops') or items.get('treestems'):
        body = body + '\n**Plants**:\n\n'
        if objs := items.get('bushes'): body = body + md.ln('**[[ Bushes|Plants#bushes ]]**: ' + get_placeable_item(objs, lib))
        if objs := items.get('grasses'): body = body + md.ln('**[[ Grasses|Plants#grasses ]]**: ' + get_placeable_item(objs, lib))
        if objs := items.get('treetops'): body = body + md.ln('**[[ Foliage|Plants#foliages ]]**: ' + get_placeable_item(objs, lib))
        if objs := items.get('treestems'): body = body + md.ln('**[[ Stems|Plants#stems ]]**: ' + get_placeable_item(objs, lib))
    return body
